...
All geometric tables are immutable after construction. `X5CubemapPipeline` creates both the stitch LUT and cubemap sampler once; `process` only remaps, blends, and optionally encodes. Detection should consume `faces` in memory. JPEG encoding is only for transport or logging because it materially reduces the real-time budget.
